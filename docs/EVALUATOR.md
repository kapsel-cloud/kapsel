...
This guide authenticates and runs the published v0.2.x `x86_64-unknown-linux-gnu` developer beta. It
...
failed-rollout, and process-loss paths. It makes the important distinctions visible:

```text
exact operator authority
  -> durable state before the mutation
  -> one conditional patch opportunity
  -> observation-only recovery after ambiguity
  -> SUCCEEDED / FAILED / UNKNOWN
  -> frozen receipt inspected under separate trust
```

`SUCCEEDED` and `FAILED` come from defined rollout observations, not patch acceptance or process
exit. `UNKNOWN` means bounded reconciliation established neither result. `NOT_ATTEMPTED` is a
separate local disposition before the mutation marker.

An `INSPECTED` receipt authenticates frozen bytes and classifier consistency under supplied trust.
It does not prove Kubernetes truth, causation, exactly-once effects, complete capture, compliance,
or production readiness.

## Fastest path

You need Docker, kind 0.32 or newer, kubectl 1.30 or newer, Python 3.11 or newer, Cosign 3.1.2,
`curl`, and GNU `sha256sum` on x86-64 GNU/Linux:
...
3. run this from the extracted top-level directory:
...
paths, and lifecycle controls remain operator-owned.

### Inspect a retained receipt offline
...
Expected trusted prototype status is `INSPECTED`, followed by the signed classifier inputs, the
recomputed `SUCCEEDED`, `FAILED`, or `UNKNOWN` result, and fixed non-claims. Structural, signature,
and external-trust failures remain distinct as `STRUCTURE_REJECTED`, `SIGNATURE_REJECTED`, and
`UNTRUSTED_SIGNER`.
