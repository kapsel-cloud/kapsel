...
The first 2026-07-23 document screen found that Google, Azure, and AWS all retain unavoidable
provider management records for 90 to 400 days. The privacy owner now distinguishes those bounded
control-plane records from Kapsel-owned audit and diagnostic data. Provider-fixed records may
outlive 24 hours only under the narrow field, access, export, and non-disclosure rules in
[Privacy](PRIVACY.md#collection-and-disclosure-rules). This change reopens managed candidates; it
does not accept one.
...
Manager Data Access, Kubernetes workload audit, Policy Denied evidence, and every configurable log
remain outside the exception and must be excluded from long-lived buckets or routed through a
bounded channel that satisfies the 24-hour rule.
