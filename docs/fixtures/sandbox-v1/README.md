...
`unavailable-image.json` points to a deterministic effect-gateway V2 receipt hex fixture and its
exact SHA-256. The signed statement uses the same synthetic operation identity and a `FAILED`
...
The unavailable-image receipt remains useful to the root offline-inspection test as
classifier-complete effect-gateway evidence.
