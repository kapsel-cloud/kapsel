...
cargo build --locked --offline -p kapsel-sandbox
production_binary="$repository_root/target/debug/kapsel-sandbox"
[[ -x "$production_binary" ]] || {
...
unshare --net env "${common_environment[@]}" KAPSEL_STAGING_TEST_ROLE=reader \
  ...
  "$test_binary" --exact "$test_name" --ignored --nocapture
state="$temporary/controller-state"
mkdir -m 0700 "$state" "$state/receipts"
chown -R "$controller_uid:$controller_gid" "$state"
unshare --net "$launcher" "$controller_uid" "$controller_gid" 0 \
  "$production_binary" init \
  --database "$state/sandbox.sqlite3" --receipts "$state/receipts" \
  --authority-root "$root" \
  --controller-uid "$controller_uid" --controller-gid "$controller_gid" \
  --staging-uid "$staging_uid" --staging-gid "$staging_gid"
